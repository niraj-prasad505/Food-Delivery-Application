    ...
    try {
        ...
        if (!mongoose.Types.ObjectId.isValid(productId)) {
            return res.status(400).json({
                success: false,
                message: "Invalid Product ID format. Mock items cannot be saved to database.",
            });
        }
...
const updateCartItem = async (req, res) => {
    try {
        ...
        await cart.save();
        await cart.populate({ path: "items.product", select: "name images price discount category stock rating" });
